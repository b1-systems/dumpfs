...
def do_large_write(INFILE, size):    
    ...
    print "create random file"
    if size > 1024**2:
        count = size/(1024**2)
        size = count * (1024**2)
        print "write %s blocks of 1M" %count
        subprocess.check_call(["dd", "if=/dev/urandom", "of=%s" % INFILE, "bs=1M", "count=%s" %count])
        #subprocess.check_call(["dd", "if=/dev/urandom", "of=%s" % INFILE, "bs=", "count=%s" %count])
    else:
...
parser = optparse.OptionParser(usage="usage: %prog [options] DUMPFILE SIZE")
